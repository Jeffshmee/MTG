# Farewell

> [!card-proxy] **`mtg:Farewell`**
>
> **Status:** Main Deck  
> **Mana Cost:** {4}{W}{W}  
> **Type:** Sorcery  
>
> ### Deck Synergy
> **88** / 100
>
> <div class="synergy-bar"><div style="width:88%"></div></div>
>
> ![Farewell](https://cards.scryfall.io/border_crop/front/4/d/4d8b6418-da30-4c68-b87c-3936eb77620c.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 92 | Game changer. Exile modes wipe tokens, counters, yards. |
> | Deck-Specific | 86 | Game changer. Exile modes wipe tokens, counters, yards. |
> | **Combined** | **88** | Game changer. Exile modes wipe tokens, counters, yards. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/White\|White]] |
> | **Type** | [[04 Types/Sorcery\|Sorcery]] |
> | **Effects** | [[03 Effects/Exile\|Exile]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Exile\|Exile]] | Choose one or more — • Exile all artifacts |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">88<span>/100</span></div><p class="synergy-score-why">Game changer. Exile modes wipe tokens, counters, yards. General 92 and Deck-Specific 86 produce Combined 88.</p></div>

## Oracle Text

Choose one or more —
• [[03 Effects/Exile|Exile all artifacts]].
• Exile all creatures.
• Exile all enchantments.
• Exile all graveyards.

---

## Deck Role & Rating

Farewell ({4}{W}{W}). Game changer. Exile modes wipe tokens, counters, yards. Judge it by whether it triggers [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]] or is worth blinking. In hand on the sideboard. Not in the 100 until a named cut.

## Play Patterns & Lines

- Cast on curve if it is a noncreature (Kykar trigger) or an ETB worth blinking.
- Blink targets in the 65: Channeler, Hunter, Cloudblazer, Charming Prince, Overseer, Kitesail, Banishing Light.
- In hand on the sideboard. Not in the 100 until a named cut.

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- **Blink (live)**: [[02 Cards/Creatures/Thassa, Deep-Dwelling|Thassa, Deep-Dwelling]], [[02 Cards/Creatures/Skycoach Conductor|Skycoach Conductor]], [[02 Cards/Creatures/Charming Prince|Charming Prince]]
- **ETB payoffs (live)**: [[02 Cards/Creatures/Aether Channeler|Aether Channeler]], [[02 Cards/Creatures/Helpful Hunter|Helpful Hunter]], [[02 Cards/Creatures/Cloudblazer|Cloudblazer]]

## Anti-synergies / Notes

- In hand on the sideboard. Not in the 100 until a named cut.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Farewell
> mana_cost: "{4}{W}{W}"
> cmc: 6
> type: Sorcery
> power: ""
> toughness: ""
> colors: ["W"]
> color_identity: ["W"]
> keywords: []
> status: Main Deck
> scores:
>   general: 92
>   deck_specific: 86
>   combined: 88
> scryfall_id: 4d8b6418-da30-4c68-b87c-3936eb77620c
> tags:
>   - card
>   - main-deck
>   - sorcerie
> ```

*Last evaluated: 2026-08-30*  
*Data source: mtg-scryfall-bulk + kykar-zephyr-commander scoring*
