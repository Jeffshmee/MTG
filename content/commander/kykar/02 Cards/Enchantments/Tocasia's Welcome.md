# Tocasia's Welcome

> [!card-proxy] **`mtg:Tocasia's Welcome`**
>
> **Status:** Sideboard  
> **Mana Cost:** {2}{W}  
> **Type:** Enchantment  
>
> ### Deck Synergy
> **84** / 100
>
> <div class="synergy-bar"><div style="width:84%"></div></div>
>
> ![Tocasia's Welcome](https://cards.scryfall.io/border_crop/front/9/0/90e38661-292a-45e7-bda9-51af53cf0f68.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 82 | Owned. Draw the first time a creature ETBs each turn. Blink turns are extra cards. |
> | Deck-Specific | 86 | Owned. Draw the first time a creature ETBs each turn. Blink turns are extra cards. |
> | **Combined** | **84** | Owned. Draw the first time a creature ETBs each turn. Blink turns are extra cards. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/White\|White]] |
> | **Type** | [[04 Types/Enchantment\|Enchantment]] |
> | **Effects** | [[03 Effects/Draw a Card\|Draw a Card]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Draw a Card\|Draw a Card]] | Whenever one or more creatures you control with mana value 3 or less enter, draw a card |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">84<span>/100</span></div><p class="synergy-score-why">Owned. Draw the first time a creature ETBs each turn. Blink turns are extra cards. General 82 and Deck-Specific 86 produce Combined 84.</p></div>

## Oracle Text

Whenever one or more creatures you control with mana value 3 or less enter, [[03 Effects/Draw a Card|draw a card]]. This ability triggers only once each turn.

---

## Deck Role & Rating

Once a turn, a creature with mana value 3 or less entering draws a card. Blink turns are extra cards. Cut 11/09/2026 for [[02 Cards/Creatures/High Fae Trickster|High Fae Trickster]]. In hand on the sideboard — not in the 100 until a named cut.

## Play Patterns & Lines

- Cast on curve: it is a noncreature, so Kykar triggers. The first cheap ETB that turn draws.
- Best with Channeler, Charming Prince, Deputy, Kitten. Helpful Hunter is already an owned cut.
- Once per turn — a second blink the same turn does not draw again.

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- **Cheap ETBs**: [[02 Cards/Creatures/Charming Prince|Charming Prince]], [[02 Cards/Creatures/Aether Channeler|Aether Channeler]], [[02 Cards/Creatures/Displacer Kitten|Displacer Kitten]], [[02 Cards/Creatures/Deputy of Detention|Deputy of Detention]]
- **Blink**: [[02 Cards/Creatures/Thassa, Deep-Dwelling|Thassa, Deep-Dwelling]], [[02 Cards/Creatures/Skycoach Conductor|Skycoach Conductor]], [[02 Cards/Instants/Ephemerate|Ephemerate]]

## Anti-synergies / Notes

- In hand on the sideboard. Not in the 100 until a named cut.
- Mana value 4+ ETBs (Mulldrifter, Cloudblazer, Drake) do not count.
- Tokens that enter do count if their mana value is 3 or less.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Tocasia's Welcome
> mana_cost: "{2}{W}"
> cmc: 3
> type: Enchantment
> power: ""
> toughness: ""
> colors: ["W"]
> color_identity: ["W"]
> keywords: []
> status: Sideboard
> scores:
>   general: 82
>   deck_specific: 86
>   combined: 84
> scryfall_id: 90e38661-292a-45e7-bda9-51af53cf0f68
> tags:
>   - card
>   - sideboard
>   - enchantment
> ```

*Last evaluated: 2026-08-30*  
*Data source: mtg-scryfall-bulk + kykar-zephyr-commander scoring*
