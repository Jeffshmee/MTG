# Wayfarer's Bauble

> [!card-proxy] **`mtg:Wayfarer's Bauble`**
>
> **Status:** Sideboard  
> **Mana Cost:** {1}  
> **Type:** Artifact  
>
> ### Deck Synergy
> **81** / 100
>
> <div class="synergy-bar"><div style="width:81%"></div></div>
>
> ![Wayfarer's Bauble](https://cards.scryfall.io/border_crop/front/a/c/ac1960a1-9141-468c-b96c-6c8e6905084d.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 80 | Owned. Land onto the battlefield. Spell that triggers Kykar. |
> | Deck-Specific | 82 | Owned. Land onto the battlefield. Spell that triggers Kykar. |
> | **Combined** | **81** | Owned. Land onto the battlefield. Spell that triggers Kykar. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Colourless\|Colourless]] |
> | **Type** | [[04 Types/Artifact\|Artifact]] |
> | **Effects** | [[03 Effects/Tutor\|Tutor]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Tutor\|Tutor]] | Search your library |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">81<span>/100</span></div><p class="synergy-score-why">Owned. Land onto the battlefield. Spell that triggers Kykar. General 80 and Deck-Specific 82 produce Combined 81.</p></div>

## Oracle Text

{2}, {T}, Sacrifice this artifact: [[03 Effects/Tutor|Search your library for a basic land card, put that card onto the battlefield tapped, then shuffle]].

---

## Deck Role & Rating

Wayfarer's Bauble ({1}). Owned. Land onto the battlefield. Spell that triggers Kykar. Judge it by whether it triggers [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]] or is worth blinking.

## Play Patterns & Lines

- Cast on curve if it is a noncreature (Kykar trigger) or an ETB worth blinking.
- Blink targets in the 65: Channeler, Hunter, Cloudblazer, Charming Prince, Overseer, Kitesail, Banishing Light.

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- **Blink (live)**: [[02 Cards/Creatures/Thassa, Deep-Dwelling|Thassa, Deep-Dwelling]], [[02 Cards/Creatures/Skycoach Conductor|Skycoach Conductor]], [[02 Cards/Creatures/Charming Prince|Charming Prince]]
- **ETB payoffs (live)**: [[02 Cards/Creatures/Aether Channeler|Aether Channeler]], [[02 Cards/Creatures/Helpful Hunter|Helpful Hunter]], [[02 Cards/Creatures/Cloudblazer|Cloudblazer]]

## Anti-synergies / Notes

- Judge it by whether it triggers Kykar or is worth blinking. Spirit mode is backup.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Wayfarer's Bauble
> mana_cost: "{1}"
> cmc: 1
> type: Artifact
> power: ""
> toughness: ""
> colors: []
> color_identity: []
> keywords: []
> status: Sideboard
> scores:
>   general: 80
>   deck_specific: 82
>   combined: 81
> scryfall_id: ac1960a1-9141-468c-b96c-6c8e6905084d
> tags:
>   - card
>   - sideboard
>   - artifact
> ```

*Last evaluated: 2026-08-30*  
*Data source: mtg-scryfall-bulk + kykar-zephyr-commander scoring*
