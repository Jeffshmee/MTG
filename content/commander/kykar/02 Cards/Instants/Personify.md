# Personify

> [!card-proxy] **`mtg:Personify`**
>
> **Status:** Sideboard  
> **Mana Cost:** {1}{W}  
> **Type:** Instant  
>
> ### Deck Synergy
> **76** / 100
>
> <div class="synergy-bar"><div style="width:76%"></div></div>
>
> ![Personify](https://cards.scryfall.io/border_crop/front/1/1/1172582d-fb2d-4022-95b1-e48b03df3a95.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 70 | Instant blink plus a changeling token. Combat-speed ETB. Do not blink the token. |
> | Deck-Specific | 80 | Instant blink plus a changeling token. Combat-speed ETB. Do not blink the token. |
> | **Combined** | **76** | Instant blink plus a changeling token. Combat-speed ETB. Do not blink the token. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/White\|White]] |
> | **Type** | [[04 Types/Instant\|Instant]] |
> | **Effects** | [[03 Effects/Exile\|Exile]] · [[03 Effects/Create Token\|Create Token]] · [[03 Effects/Blink\|Blink]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Exile\|Exile]] | Exile target creature you control, then return that card to the battlefield under its owner's control |
> | [[03 Effects/Create Token\|Create Token]] | Create a 1/1 colorless Shapeshifter creature token with changeling |
> | [[03 Effects/Blink\|Blink]] | Exile then return to the battlefield |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">76<span>/100</span></div><p class="synergy-score-why">Instant blink plus a changeling token. Combat-speed ETB. Do not blink the token. General 70 and Deck-Specific 80 produce Combined 76.</p></div>

## Oracle Text

[[03 Effects/Exile|[[03 Effects/Blink|Exile target creature you control, then return that card to the battlefield under its owner's control]]]]. [[03 Effects/Create Token|Create a 1/1 colorless Shapeshifter creature token with changeling]]. (It's every creature type.)

---

## Deck Role & Rating

Instant blink a creature you control, then a 1/1 changeling. Combat-speed ETB for Channeler or Hunter. Do not blink the token — it ceases. Live in the 70.

## Play Patterns & Lines

- Cast after an ETB creature is in play. Empty-board blink is a miss.
- Instant-speed (this card, Skycoach, Kitten) on their attack; Kykar/Thassa wait until end of step.
- Priority targets: Channeler (bounce the engine), Hunter / Cloudblazer (draw), Banishing Light (new exile).

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- **Blink (live)**: [[02 Cards/Creatures/Thassa, Deep-Dwelling|Thassa, Deep-Dwelling]], [[02 Cards/Creatures/Skycoach Conductor|Skycoach Conductor]], [[02 Cards/Creatures/Charming Prince|Charming Prince]]
- **ETB payoffs (live)**: [[02 Cards/Creatures/Aether Channeler|Aether Channeler]], [[02 Cards/Creatures/Helpful Hunter|Helpful Hunter]], [[02 Cards/Creatures/Cloudblazer|Cloudblazer]]

## Anti-synergies / Notes

- Your own tokens that leave (Kykar Spirit, Channeler Bird) cease. Do not blink a token expecting it back.
- Kykar's blink is delayed. Do not block with the blinked creature; it is gone until end of turn.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Kykar, Zephyr Awakener|Kykar, Zephyr Awakener]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Personify
> mana_cost: "{1}{W}"
> cmc: 2
> type: Instant
> power: ""
> toughness: ""
> colors: ["W"]
> color_identity: ["W"]
> keywords: []
> status: Sideboard
> scores:
>   general: 70
>   deck_specific: 80
>   combined: 76
> scryfall_id: 1172582d-fb2d-4022-95b1-e48b03df3a95
> tags:
>   - card
>   - sideboard
>   - instant
> ```

*Last evaluated: 2026-08-30*  
*Data source: mtg-scryfall-bulk + kykar-zephyr-commander scoring*
