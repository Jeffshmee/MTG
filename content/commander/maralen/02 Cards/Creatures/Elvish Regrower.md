# Elvish Regrower

> [!card-proxy] **`mtg:Elvish Regrower`**
>
> **Status:** Main Deck  
> **Mana Cost:** {2}{G}{G}  
> **Type:** Creature — Elf Druid  
> **P/T:** 4/3  
>
> ### Deck Synergy
> **76** / 100
>
> <div class="synergy-bar"><div style="width:76%"></div></div>
>
> ![Elvish](https://cards.scryfall.io/border_crop/front/2/6/2694e3cd-26ed-4a10-ae55-fb84d7800253.jpg?1783909097)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 74 | Elf that returns a permanent. A bit expensive. |
> | Deck-Specific | 78 | Elf that returns a permanent. A bit expensive. |
> | **Combined** | **76** | Elf that returns a permanent. A bit expensive. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Green\|Green]] |
> | **Type** | [[04 Types/Creature\|Creature]] · [[04 Types/Elf\|Elf]] · [[04 Types/Druid\|Druid]] |
> | **Effects** | [[03 Effects/ETB\|ETB]] · [[03 Effects/Recursion\|Recursion]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/ETB\|ETB]] | When this creature enters |
> | [[03 Effects/Recursion\|Recursion]] | Return target permanent card from your graveyard |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">76<span>/100</span></div><p class="synergy-score-why">Elf that returns a permanent. A bit expensive. General 74 and Deck-Specific 78 produce Combined 76.</p></div>

## Oracle Text

[[03 Effects/ETB|When this creature enters]], [[03 Effects/Recursion|return target permanent card from your graveyard]] to your hand.

---

## Deck Role & Rating

Regrower is the cheaper permanent-recursion Elf. Same ETB as [[02 Cards/Creatures/Golgari Findbroker|Golgari Findbroker]] — any permanent card from the yard to hand — for `{2}{G}{G}` and a 4/3 body. Priority targets are [[02 Cards/Creatures/High Perfect Morcant|High Perfect Morcant]], [[02 Cards/Creatures/Lathril, Blade of the Elves|Lathril]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]], then [[02 Cards/Enchantments/Garruk's Uprising|Garruk's Uprising]], [[02 Cards/Artifacts/Patchwork Banner|Patchwork Banner]], and [[02 Cards/Artifacts/Panharmonicon|Panharmonicon]] (and [[02 Cards/Enchantments/Kindred Discovery|Kindred Discovery]] once it arrives). He mills and counts, but the returned card is why he is in the pile.

## Play Patterns & Lines

- After Maralen, a four-drop that mills *and* puts Morcant or Lathril back in hand is the correct sequence.
- Prefer him over Findbroker when you only have green. Save Findbroker for the second trip or when you need the extra toughness.
- [[02 Cards/Sorceries/Genesis Wave|Wave]] that hits Regrower can bounce a legend that died earlier in the same turn or a Banner/Uprising the Wave overwrote.
- Do not return a basic land unless the yard is empty of engines.

## Key Synergies

- **Replay the legends**: [[02 Cards/Creatures/High Perfect Morcant|High Perfect Morcant]], [[02 Cards/Creatures/Lathril, Blade of the Elves|Lathril, Blade of the Elves]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]]
- **Permanent engines**: [[02 Cards/Enchantments/Garruk's Uprising|Garruk's Uprising]], [[02 Cards/Artifacts/Patchwork Banner|Patchwork Banner]], [[02 Cards/Artifacts/Panharmonicon|Panharmonicon]], [[02 Cards/Enchantments/Kindred Discovery|Kindred Discovery]]
- **Fill the yard**: [[02 Cards/Creatures/Glowspore Shaman|Glowspore Shaman]], [[02 Cards/Creatures/Lluwen, Imperfect Naturalist|Lluwen]], [[02 Cards/Creatures/Lys Alana Informant|Lys Alana Informant]]
- **On-death Elf only**: [[02 Cards/Creatures/Morcant's Loyalist|Morcant's Loyalist]]

## Anti-synergies / Notes

- Four mana for a 4/3 that does nothing if the yard is empty.
- Cannot return instants or sorceries.
- He is not a dork. Do not curve him as ramp.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Elvish Regrower
> mana_cost: "{2}{G}{G}"
> cmc: 4.0
> type: Creature — Elf Druid
> power: "4"
> toughness: "3"
> colors: [G]
> color_identity: [G]
> keywords: []
> status: Main Deck
> scores:
>   general: 74
>   deck_specific: 78
>   combined: 76
> scryfall_id: 2694e3cd-26ed-4a10-ae55-fb84d7800253
> tags:
>   - card
>   - main-deck
>   - creature
> ```

*Last evaluated: 2026-08-17*  
*Data source: mtg-scryfall-bulk + maralen-fae-commander scoring*
