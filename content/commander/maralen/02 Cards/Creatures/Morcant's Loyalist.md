# Morcant's Loyalist

> [!card-proxy] **`mtg:Morcant's Loyalist`**
>
> **Status:** Main Deck  
> **Mana Cost:** {1}{B}{G}  
> **Type:** Creature — Elf Warrior  
> **P/T:** 3/2  
>
> ### Deck Synergy
> **86** / 100
>
> <div class="synergy-bar"><div style="width:86%"></div></div>
>
> ![Morcant's](https://cards.scryfall.io/border_crop/front/1/7/175c6b1c-8790-41a2-ae15-5031206c410f.jpg?1783904405)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 82 | Elf anthem and recursion. Directly feeds the plan. |
> | Deck-Specific | 88 | Elf anthem and recursion. Directly feeds the plan. |
> | **Combined** | **86** | Elf anthem and recursion. Directly feeds the plan. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Black\|Black]] · [[05 Colours/Green\|Green]] |
> | **Type** | [[04 Types/Creature\|Creature]] · [[04 Types/Elf\|Elf]] · [[04 Types/Warrior\|Warrior]] |
> | **Effects** | [[03 Effects/Pump Creatures\|Pump Creatures]] · [[03 Effects/Dies Trigger\|Dies Trigger]] · [[03 Effects/Recursion\|Recursion]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Pump Creatures\|Pump Creatures]] | Other Elves you control get +1/+1 |
> | [[03 Effects/Dies Trigger\|Dies Trigger]] | Dies |
> | [[03 Effects/Recursion\|Recursion]] | Return another target Elf card from your graveyard |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">86<span>/100</span></div><p class="synergy-score-why">Elf anthem and recursion. Directly feeds the plan. General 82 and Deck-Specific 88 produce Combined 86.</p></div>

## Oracle Text

Other Elves you control [[03 Effects/Pump Creatures|get +1/+1]].
When this creature [[03 Effects/Dies Trigger|dies]], [[03 Effects/Recursion|return another target Elf card from your graveyard]] to your hand.

---

## Deck Role & Rating

Loyalist is the Elf-only recursion piece and a small anthem. Other Elves get +1/+1 while he lives; when he dies he returns *another* Elf card from the yard to hand. That is how you replay [[02 Cards/Creatures/High Perfect Morcant|High Perfect Morcant]], [[02 Cards/Creatures/Lathril, Blade of the Elves|Lathril]], or [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]] after a fight or a wipe. He does **not** get Banner, Uprising, or Panharmonicon — that is [[02 Cards/Creatures/Golgari Findbroker|Findbroker]].

## Play Patterns & Lines

- Cast him after Maralen so the ETB mills. The anthem makes Lathril connects and combat math real.
- Trade him in combat, or let him die to a wipe, only if Morcant / Lathril / Archdruid is already in the yard. The dies trigger cannot return himself.
- Chain: Loyalist dies → return Findbroker or Regrower → that ETB returns a non-Elf engine.
- He is a three-drop, not a dork. Do not curve him instead of a mana Elf.

## Key Synergies

- **Elf recursion targets**: [[02 Cards/Creatures/High Perfect Morcant|High Perfect Morcant]], [[02 Cards/Creatures/Lathril, Blade of the Elves|Lathril, Blade of the Elves]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]], [[02 Cards/Creatures/Imperious Perfect|Imperious Perfect]]
- **Chain into permanents**: [[02 Cards/Creatures/Golgari Findbroker|Golgari Findbroker]]
- **Anthem stack**: [[02 Cards/Creatures/Imperious Perfect|Imperious Perfect]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]], [[02 Cards/Creatures/Leaf-Crowned Visionary|Leaf-Crowned Visionary]], [[02 Cards/Artifacts/Patchwork Banner|Patchwork Banner]]
- **Anthem stack**: [[02 Cards/Creatures/Imperious Perfect|Imperious Perfect]], [[02 Cards/Artifacts/Patchwork Banner|Patchwork Banner]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]]
- **Make him die on purpose**: combat, blight trades via Morcant, a wipe you are already losing

## Anti-synergies / Notes

- Exile-based removal skips the dies trigger.
- Cannot return Banner, Uprising, Panharmonicon, or Kindred Discovery.
- If the only Elf in the yard is a dork, the trigger is still fine — just do not pretend it is a Morcant.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Morcant's Loyalist
> mana_cost: "{1}{B}{G}"
> cmc: 3.0
> type: Creature — Elf Warrior
> power: "3"
> toughness: "2"
> colors: [B, G]
> color_identity: [B, G]
> keywords: []
> status: Main Deck
> scores:
>   general: 82
>   deck_specific: 88
>   combined: 86
> scryfall_id: 175c6b1c-8790-41a2-ae15-5031206c410f
> tags:
>   - card
>   - main-deck
>   - creature
> ```

*Last evaluated: 2026-08-17*  
*Data source: mtg-scryfall-bulk + maralen-fae-commander scoring*
