# Priest of Titania

> [!card-proxy] **`mtg:Priest of Titania`**
>
> **Status:** Main Deck 
> **Mana Cost:** {1}{G} 
> **Type:** Creature — Elf Druid 
> **P/T:** 1/1 
>
> ### Deck Synergy
> **91** / 100
>
> <div class="synergy-bar"><div style="width:91%"></div></div>
>
> ![Priest](https://cards.scryfall.io/border_crop/front/e/b/eb11921b-1b28-483f-a707-4de21a6daa31.jpg?1783911212)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 86 | Elf that turns the board into Maralen-casting mana. |
> | Deck-Specific | 94 | Elf that turns the board into Maralen-casting mana. |
> | **Combined** | **91** | Elf that turns the board into Maralen-casting mana. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Green\|Green]] |
> | **Type** | [[04 Types/Creature\|Creature]] · [[04 Types/Elf\|Elf]] · [[04 Types/Druid\|Druid]] |
> | **Effects** | [[03 Effects/Add Mana\|Add Mana]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Add Mana\|Add Mana]] | {T}: Add {G} for each Elf on the battlefield |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">91<span>/100</span></div><p class="synergy-score-why">Elf that turns the board into Maralen-casting mana. General 86 and Deck-Specific 94 produce Combined 91.</p></div>

## Oracle Text

{T}: [[03 Effects/Add Mana|Add]] {G} for each Elf on the battlefield.

---

## Deck Role & Rating

Priest is the live Elf-count ritual: tap for {G} per Elf on the battlefield, including opponents'. That is the same Wave math as [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]] — X equals Elves minus 3 — at two mana instead of three, but with no anthem and no "you control" limit. Faeries do not pay the tap; [[02 Cards/Creatures/Circle of Dreams Druid|Circle of Dreams Druid]] is live and counts every creature, including Faeries.

## Play Patterns & Lines

- Cast it onto a few Elves so the first tap is already three-plus green, then hold for [[02 Cards/Sorceries/Genesis Wave|Genesis Wave]].
- Count opponents' Elves. Tribal tables inflate the ritual; empty tables do not.
- Use the burst to hit Maralen if you are short, then keep the tap for Wave once she is up.
- When it dies, [[02 Cards/Creatures/Golgari Findbroker|Golgari Findbroker]] or [[02 Cards/Creatures/Elvish Regrower|Elvish Regrower]] get any permanent back; [[02 Cards/Creatures/Morcant's Loyalist|Morcant's Loyalist]] also works because Priest is an Elf.

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- **Wave**: [[02 Cards/Sorceries/Genesis Wave|Genesis Wave]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]]
- **Ritual that counts Faeries**: [[02 Cards/Creatures/Circle of Dreams Druid|Circle of Dreams Druid]]
- **Elf tokens (owned)**: [[02 Cards/Creatures/Imperious Perfect|Imperious Perfect]], [[02 Cards/Creatures/Lathril, Blade of the Elves|Lathril, Blade of the Elves]]
- **Tutor**: [[02 Cards/Sorceries/Nature's Rhythm|Nature's Rhythm]] (X=2)
- **Recursion**: [[02 Cards/Creatures/Golgari Findbroker|Golgari Findbroker]], [[02 Cards/Creatures/Elvish Regrower|Elvish Regrower]], [[02 Cards/Creatures/Morcant's Loyalist|Morcant's Loyalist]]

## Anti-synergies / Notes

- Counts Elves only. Faerie tokens from Bearer or Alela do not add mana.
- 1/1. Any ping removes the ritual until you recur it.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Priest of Titania
> mana_cost: "{1}{G}"
> cmc: 2.0
> type: Creature — Elf Druid
> power: "1"
> toughness: "1"
> colors: [G]
> color_identity: [G]
> keywords: []
> status: Main Deck
> scores:
>  general: 86
>  deck_specific: 94
>  combined: 91
> scryfall_id: eb11921b-1b28-483f-a707-4de21a6daa31
> tags:
>  - card
>  - main-deck
>  - creature
> ```

*Last evaluated: 2026-08-17* 
*Data source: mtg-scryfall-bulk + maralen-fae-commander scoring*
