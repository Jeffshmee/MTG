# Patchwork Banner

> [!card-proxy] **`mtg:Patchwork Banner`**
>
> **Status:** Main Deck  
> **Mana Cost:** {3}  
> **Type:** Artifact  
>
> ### Deck Synergy
> **85** / 100
>
> <div class="synergy-bar"><div style="width:85%"></div></div>
>
> ![Patchwork](https://cards.scryfall.io/border_crop/front/8/1/81669ca8-3f20-45e6-b088-65969dcd93d1.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 78 | Name Elf: anthem plus any-colour mana. Theme piece. |
> | Deck-Specific | 90 | Name Elf: anthem plus any-colour mana. Theme piece. |
> | **Combined** | **85** | Name Elf: anthem plus any-colour mana. Theme piece. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Colourless\|Colourless]] |
> | **Type** | [[04 Types/Artifact\|Artifact]] |
> | **Effects** | [[03 Effects/Pump Creatures\|Pump Creatures]] · [[03 Effects/Choose Type\|Choose Type]] · [[03 Effects/Add Mana\|Add Mana]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Pump Creatures\|Pump Creatures]] | Creatures you control of the chosen type get +1/+1 |
> | [[03 Effects/Choose Type\|Choose Type]] | Choose a creature type |
> | [[03 Effects/Add Mana\|Add Mana]] | {T}: Add one mana of any color |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">85<span>/100</span></div><p class="synergy-score-why">Name Elf: anthem plus any-colour mana. Theme piece. General 78 and Deck-Specific 90 produce Combined 85.</p></div>

## Oracle Text

As this artifact enters, [[03 Effects/Choose Type|choose a creature type]].
Creatures you control of the chosen type [[03 Effects/Pump Creatures|get +1/+1]].
{T}: [[03 Effects/Add Mana|Add]] one mana of any color.

---

## Deck Role & Rating

Banner is why the list is Elf-majority. Name Elf: anthem on the team plus any-colour mana for Maralen. It is a first-priority permanent for [[02 Cards/Creatures/Golgari Findbroker|Findbroker]] / [[02 Cards/Creatures/Elvish Regrower|Regrower]] and a Wave hit that immediately turns the rest of the dump sideways. It is not an Elf itself — it does not mill.

## Play Patterns & Lines

- Name Elf. Always. The owned pile is not a Faerie deck.
- Curve it before Maralen if you need the coloured mana; after her if you already have {B}{G}{U} and want the anthem on Lathril / tokens.
- If it dies, get it back before you get a dork. Lords without Banner still work; Banner without a board is still a rock.

## Key Synergies

- **The team it pumps**: [[02 Cards/Creatures/Lathril, Blade of the Elves|Lathril]], [[02 Cards/Creatures/Imperious Perfect|Imperious Perfect]], [[02 Cards/Creatures/Elvish Archdruid|Elvish Archdruid]]
- **Replay it**: [[02 Cards/Creatures/Golgari Findbroker|Golgari Findbroker]], [[02 Cards/Creatures/Elvish Regrower|Elvish Regrower]]
- **Wave**: a 3-mana hit that turns the rest of the pile into attackers

## Anti-synergies / Notes

- Naming Faerie is a mistake on this owned list.
- Does not trigger Maralen. Do not count it as a body.
- Changelings (Firdoch) get the pump. Servos and Worms do too if you named the wrong type — don't.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Patchwork Banner
> mana_cost: "{3}"
> cmc: 3.0
> type: Artifact
> power: ""
> toughness: ""
> colors: []
> color_identity: []
> keywords: []
> status: Main Deck
> scores:
>   general: 78
>   deck_specific: 90
>   combined: 85
> scryfall_id: 81669ca8-3f20-45e6-b088-65969dcd93d1
> tags:
>   - card
>   - main-deck
>   - artifact
> ```

*Last evaluated: 2026-08-17*  
*Data source: mtg-scryfall-bulk + maralen-fae-commander scoring*
