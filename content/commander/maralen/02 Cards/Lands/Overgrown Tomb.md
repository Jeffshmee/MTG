# Overgrown Tomb

> [!card-proxy] **`mtg:Overgrown Tomb`**
>
> **Status:** Main Deck  
> **Mana Cost:** —  
> **Type:** Land — Swamp Forest  
>
> ### Deck Synergy
> **84** / 100
>
> <div class="synergy-bar"><div style="width:84%"></div></div>
>
> ![Overgrown](https://cards.scryfall.io/border_crop/front/4/5/45b92924-baa1-4c9b-9932-9a5eda8f3446.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 86 | Shock dual in the two heaviest colours. |
> | Deck-Specific | 82 | Shock dual in the two heaviest colours. |
> | **Combined** | **84** | Shock dual in the two heaviest colours. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Black\|Black]] · [[05 Colours/Green\|Green]] |
> | **Type** | [[04 Types/Land\|Land]] · [[04 Types/Swamp\|Swamp]] · [[04 Types/Forest\|Forest]] |
> | **Effects** | [[03 Effects/Enters Tapped\|Enters Tapped]] · [[03 Effects/Pain Land\|Pain Land]] · [[03 Effects/Add Mana\|Add Mana]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Enters Tapped\|Enters Tapped]] | Enters tapped |
> | [[03 Effects/Pain Land\|Pain Land]] | You may pay 2 life |
> | [[03 Effects/Add Mana\|Add Mana]] | {T}: Add {B} or {G} |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">84<span>/100</span></div><p class="synergy-score-why">Shock dual in the two heaviest colours. General 86 and Deck-Specific 82 produce Combined 84.</p></div>

## Oracle Text

({T}: [[03 Effects/Add Mana|Add]] {B} or {G}.)
As this land enters, [[03 Effects/Pain Land|you may pay 2 life]]. If you don't, it [[03 Effects/Enters Tapped|enters tapped]].

---

## Deck Role & Rating

Overgrown Tomb is fixing that comes in tapped. Fine while the owned list is short; premium duals on order replace the worst of these.

## Play Patterns & Lines

- Sequence it so Maralen is {B}{G}{U} on time. Name Elf on tribal lands.

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- **Ramp**: helps hit Maralen on time.

## Anti-synergies / Notes

- Keep Maralen on the battlefield. The deck does much less without her.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Overgrown Tomb
> mana_cost: ""
> cmc: 0.0
> type: Land — Swamp Forest
> power: ""
> toughness: ""
> colors: [B, G]
> color_identity: [B, G]
> keywords: []
> status: Main Deck
> scores:
>   general: 86
>   deck_specific: 82
>   combined: 84
> scryfall_id: 45b92924-baa1-4c9b-9932-9a5eda8f3446
> tags:
>   - card
>   - main-deck
>   - land
> ```

*Last evaluated: 2026-08-17*  
*Data source: mtg-scryfall-bulk + maralen-fae-commander scoring*
