# Nature's Lore

> [!card-proxy] **`mtg:Nature's Lore`**
>
> **Status:** Main Deck  
> **Mana Cost:** {1}{G}  
> **Type:** Sorcery  
>
> ### Deck Synergy
> **81** / 100
>
> <div class="synergy-bar"><div style="width:81%"></div></div>
>
> ![Nature's Lore](https://cards.scryfall.io/border_crop/front/7/a/7afc4a15-5ff8-457a-aa62-803b2d78d912.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 82 | Untapped dual fetch. Land-quality upgrade. |
> | Deck-Specific | 80 | Untapped dual fetch. Land-quality upgrade. |
> | **Combined** | **81** | Untapped dual fetch. Land-quality upgrade. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Green\|Green]] |
> | **Type** | [[04 Types/Sorcery\|Sorcery]] |
> | **Effects** | [[03 Effects/Tutor\|Tutor]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Tutor\|Tutor]] | Search your library |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">81<span>/100</span></div><p class="synergy-score-why">Untapped dual fetch. Land-quality upgrade. General 82 and Deck-Specific 80 produce Combined 81.</p></div>

## Oracle Text

[[03 Effects/Tutor|Search your library]] for a Forest card, put that card onto the battlefield, then shuffle.

---

## Deck Role & Rating

Search for a Forest card and put it onto the battlefield untapped. That includes duals: [[02 Cards/Lands/Overgrown Tomb|Overgrown Tomb]], [[02 Cards/Lands/Breeding Pool|Breeding Pool]]. Faster than [[02 Cards/Sorceries/Farseek|Farseek]] because the land can tap the same turn. Same job as [[02 Cards/Sorceries/Three Visits|Three Visits]]. Arrived 11/09/2026. In hand on the sideboard. Not in the 100 until a named cut.

## Play Patterns & Lines

- Fetch Tomb or Breeding Pool if it is still in the 99. Otherwise a basic Forest.
- Untapped means you can still drop a dork or hold [[02 Cards/Instants/Negate|Negate]].
- Does not find Island or [[02 Cards/Lands/Watery Grave|Watery Grave]]. Use Farseek for those.

## Key Synergies

- **Untapped Forests**: [[02 Cards/Lands/Overgrown Tomb|Overgrown Tomb]], [[02 Cards/Lands/Breeding Pool|Breeding Pool]], Forest
- **Same card, other name**: [[02 Cards/Sorceries/Three Visits|Three Visits]]
- **Commander**: [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]

## Anti-synergies / Notes

- In hand on the sideboard. Not in the 100 until a named cut.
- Cannot fetch Island or Watery Grave.
- A Forest card in the library, not "a land that is a Forest on the battlefield." [[02 Cards/Lands/Yavimaya, Cradle of Growth|Yavimaya]] does not change what this can find.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Maralen, Fae Ascendant|Maralen, Fae Ascendant]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Nature's Lore
> mana_cost: "{1}{G}"
> cmc: 2.0
> type: Sorcery
> power: ""
> toughness: ""
> colors: [G]
> color_identity: [G]
> keywords: []
> status: Main Deck
> scores:
>   general: 82
>   deck_specific: 80
>   combined: 81
> scryfall_id: 7afc4a15-5ff8-457a-aa62-803b2d78d912
> tags:
>   - card
>   - main-deck
>   - sorcerie
> ```

*Last evaluated: 2026-08-17*  
*Data source: mtg-scryfall-bulk + maralen-fae-commander scoring*
