# Fabled Passage

> [!card-proxy] **`mtg:Fabled Passage`**
>
> **Status:** Main Deck  
> **Mana Cost:** —  
> **Type:** Land  
>
> ### Deck Synergy
> **80** / 100
>
> <div class="synergy-bar"><div style="width:80%"></div></div>
>
> ![Fabled Passage](https://cards.scryfall.io/border_crop/front/8/8/8809830f-d8e1-4603-9652-0ad8b00234e9.jpg)
>
> ### Deck Scores
>
> | Score | Value | Breakdown |
> |-------|-------|-----------|
> | General | 78 | Named land for the manabase. Colour fixing or a utility hole. |
> | Deck-Specific | 82 | Named land for the manabase. Colour fixing or a utility hole. |
> | **Combined** | **80** | Named land for the manabase. Colour fixing or a utility hole. |
>
> ### Classification
>
> | Category | Links |
> |----------|-------|
> | **Colours** | [[05 Colours/Colourless\|Colourless]] |
> | **Type** | [[04 Types/Land\|Land]] |
> | **Effects** | [[03 Effects/Tutor\|Tutor]] |
>
> ### Extracted Effects
>
> | Effect | Notes |
> |--------|-------|
> | [[03 Effects/Tutor\|Tutor]] | Search your library |

### Combined Deck Synergy Score

<div class="synergy-score"><div class="synergy-score-num">80<span>/100</span></div><p class="synergy-score-why">Named land for the manabase. Colour fixing or a utility hole. General 78 and Deck-Specific 82 produce Combined 80.</p></div>

## Oracle Text

{T}, Sacrifice this land: [[03 Effects/Tutor|Search your library for a basic land card, put it onto the battlefield tapped, then shuffle]]. Then if you control four or more lands, untap that land.

---

## Deck Role & Rating

[[02 Cards/Lands/Fabled Passage|Fabled Passage]] cracks for a basic, then untaps that basic if you have four or more lands. Fetch that plays as a fifth land, not a second. Find Plains or Swamp.

## Play Patterns & Lines

- Crack on 4+ lands so the basic untaps. Early crack is a tapped basic — same as Evolving Wilds.
- Find the colour you are missing for Ayli exile ({{W}}{{B}} plus {{1}}).

## Key Synergies

- **Commander**: [[02 Cards/Creatures/Ayli, Eternal Pilgrim|Ayli, Eternal Pilgrim]]
- **Basics**: [[02 Cards/Lands/Plains|Plains]], [[02 Cards/Lands/Swamp|Swamp]], [[02 Cards/Lands/Evolving Wilds|Evolving Wilds]], [[02 Cards/Lands/Terramorphic Expanse|Terramorphic Expanse]]

## Anti-synergies / Notes

- Does not find Shadowy Backstreet. Basics only.

## Related Pages

- [[01 Deck/Current Deck|Current Deck]]
- [[01 Deck/Sideboard|Sideboard]]
- [[02 Cards/Creatures/Ayli, Eternal Pilgrim|Ayli, Eternal Pilgrim]]
- [[06 Strategy/Win Conditions|Win Conditions]]
- [[06 Strategy/Play Patterns|Play Patterns]]

---

> [!metadata]- Metadata
> ```yaml
> name: Fabled Passage
> mana_cost: ""
> cmc: 0
> type: Land
> power: ""
> toughness: ""
> colors: []
> color_identity: []
> keywords: []
> status: Main Deck
> scores:
>   general: 78
>   deck_specific: 82
>   combined: 80
> scryfall_id: 8809830f-d8e1-4603-9652-0ad8b00234e9
> tags:
>   - card
>   - main-deck
>   - land
> ```

*Last evaluated: 2026-09-10*  
*Data source: mtg-scryfall-bulk + ayli-pilgrim-commander scoring*
